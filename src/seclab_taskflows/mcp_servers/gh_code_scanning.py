...
async def call_api(url: str, params: dict) -> str | httpx.Response:
    ...
    return await _fetch_alerts(url, headers=headers, params=params)


@mcp.tool()
async def get_alert_by_number(
    owner: str = Field(description="The owner of the repo"),
    repo: str = Field(description="The repository name."),
    alert_number: int = Field(description="The alert number to get the alert for. Example: 1"),
) -> str:
...
async def fetch_alerts(
    ...
    rule: str = Field(description="The rule of the alert to fetch", default=""),
) -> str:
    """Fetch all code scanning alerts for a specific repository."""
...
async def fetch_alerts_to_sql(
    ...
    state: str = Field(default="open", description="The state of the alert to filter by. Default is 'open'."),
    rule: str = Field(description="The rule of the alert to fetch", default=""),
    rename_repo: str = Field(
        description="An optional alternative repo name for storing the alerts, if not specify, repo is used ",
        default="",
    ),
) -> str:
...
async def _fetch_codeql_databases(owner: str, repo: str, language: str):
    ...
    try:
        ...
        return json.dumps(
            {
                "message": f"CodeQL database for {repo} ({language}) fetched successfully.",
                "relative_database_path": f"{owner}/{repo}/{qldb_subfolder}",
            }
        )
    except httpx.RequestError as e:
...
async def dismiss_alert(
    ...
    async with httpx.AsyncClient(headers=headers) as client:
        response = await client.patch(
            url, json={"state": "dismissed", "dismissed_reason": "false positive", "dismissed_comment": reason}
        )
        response.raise_for_status()
...
async def fetch_issues_matches(
    repo: str = Field(
        description="A comma separated list of repositories to search in. Each term is of the form owner/repo. For example: 'owner1/repo1,owner2/repo2'"
    ),
    ...
    labels: str = Field(default="", description="Labels to filter issues by"),
) -> str:
    """
