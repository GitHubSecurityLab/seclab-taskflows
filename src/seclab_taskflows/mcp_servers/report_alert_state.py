...
def remove_line_numbers(location: str) -> str:
    ...
    min_parts = 4
    if len(parts) < min_parts:  # Ensure there are enough parts to remove line numbers
        return location
...
class ReportAlertStateBackend:
    ...
    def get_completed_alerts(self, rule: str, repo: str | None = None) -> Any:
        ...
        with Session(self.engine) as session:
            return [result_to_dict(r) for r in session.query(AlertResults).filter_by(**filter_params).all()]

    def clear_completed_alerts(self, repo: str | None = None, rule: str | None = None) -> str:
    ...
    def get_alert_results_by_rule(self, rule: str, repo: str | None = None, *, valid: bool | None = None) -> Any:
        filter_params = {"rule": rule}
        if repo:
            filter_params["repo"] = repo
    ...
    def delete_alert_result(self, alert_id: str, repo: str) -> str:
        with Session(self.engine) as session:
            session.query(AlertResults).filter_by(alert_id=alert_id, repo=repo).delete()
            session.commit()
        return f"Deleted alert result for {alert_id} in {repo}"

    def clear_alert_results(self, repo: str | None = None, rule: str | None = None) -> str:
        filter_params = {}
        if repo:
            filter_params["repo"] = repo
        if rule:
    ...
    def add_flow_to_alert(
        self, canonical_id: int, flow_data: str, repo: str, prev: str | None = None, next_flow: str | None = None
    ) -> str:
    ...
    def batch_add_flow_to_alert(
        self, alert_canonical_id: int, flows: list[str], repo: str, prev: str, next_flow: str
    ) -> str:
    ...
    def delete_flow_graph_for_alert(self, alert_canonical_id: int) -> str:
        ...
        with Session(self.engine) as session:
            ...
            session.commit()
        return (
            f"Deleted flow graph with for alert with canonical iD {alert_canonical_id}"
            if result
            else "No flow graph found to delete."
        )

    def update_all_alert_results_for_flow_graph(self, next_flow: str, repo: str, result: str) -> str:
        with Session(self.engine) as session:
            flow_graphs = session.query(AlertFlowGraph).filter_by(next=next_flow, repo=repo).all()
            if not flow_graphs:
                return f"No flow graphs found with next value {next_flow}"
            alert_canonical_ids = {fg.alert_canonical_id for fg in flow_graphs}
            ...
            session.commit()
        return f"Updated alert results for flow graphs with next value {next_flow} with result: {result}"

    def delete_flow_graph(self, flow_id: int) -> str:
        """Delete a flow graph for with an id."""
        with Session(self.engine) as session:
            result = session.query(AlertFlowGraph).filter_by(id=flow_id).delete()
            session.commit()
        return f"Deleted flow graph with ID {flow_id}" if result else "No flow graph found to delete."
...
def set_alert_valid(alert_id: str, repo: str, *, valid: bool) -> str:
    ...
    return backend.set_alert_valid(alert_id, process_repo(repo), valid=valid)
...
def set_alert_completed(alert_id: str, repo: str = Field(description="repo in the format owner/repo")) -> str:
    ...
    return backend.set_alert_completed(alert_id, process_repo(repo), completed=True)
...
def clear_completed_alerts(
    repo: str | None = Field(description="repo in the format owner/repo", default=None), rule: str | None = None
) -> str:
    """Clear all completed alerts in a repository."""
...
def add_flow_to_alert(
    ...
    next_flow_clean = remove_line_numbers(next_flow) if next_flow else None
    backend.add_flow_to_alert(canonical_id, flow_data, process_repo(repo), prev, next_flow_clean)
    return f"Added flow graph for alert with canonical ID {canonical_id}"
...
def batch_add_flow_to_alert(
    ...
    return backend.batch_add_flow_to_alert(alert_canonical_id, flows_list, process_repo(repo), prev, next_flow)
...
@mcp.tool()
def delete_flow_graph(flow_id: int) -> str:
    """Delete a flow graph with id."""
    return backend.delete_flow_graph(flow_id)
...
def update_all_alert_results_for_flow_graph(
    next_flow: str, result: str, repo: str = Field(description="repo in the format owner/repo")
) -> str:
    ...
    next_flow_clean = remove_line_numbers(next_flow) if next_flow else None
    return backend.update_all_alert_results_for_flow_graph(next_flow_clean, process_repo(repo), result)
