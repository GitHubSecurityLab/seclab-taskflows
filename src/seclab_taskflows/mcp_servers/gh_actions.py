...
class WorkflowUses(Base):
    ...
    def __repr__(self):
        return f"<WorkflowUses(user={self.user}, lines={self.lines}, action_name={self.action_name}, repo={self.repo})>"


mcp = FastMCP("GitHubCodeScanning")

high_privileged_triggers = {
    "issues",
    "issue_comment",
...
async def call_api(url: str, params: dict, *, raw=False) -> str:
    ...
    return await _fetch(url, headers=headers, params=params)
...
async def fetch_workflow(
    ...
    workflow_id: str = Field(description="The ID or name of the workflow"),
) -> str:
    """
    Fetch the details of a GitHub Actions workflow.
    """
    r = await call_api(url=f"https://api.github.com/repos/{owner}/{repo}/actions/workflows/{workflow_id}", params={})
    if isinstance(r, str):
        return r
    return r.json()
...
async def get_workflow_triggers(owner: str, repo: str, workflow_file_path: str) -> str:
    r = await call_api(
        url=f"https://api.github.com/repos/{owner}/{repo}/contents/{workflow_file_path}", params={}, raw=True
    )
    if isinstance(r, str):
        return json.dumps([r])
    data = yaml.safe_load(r.text)
    #'on' is parsed as 'True': https://github.com/yaml/pyyaml/issues/470
    return list(find_in_yaml(True, data))
...
async def find_workflow_run_dependency(
    ...
    *,
    ...
    if isinstance(r, str):
        return json.dumps([r])
    data = yaml.safe_load(r.text)
    trigger_workflow = next(iter(find_in_yaml("workflow_run", data))).get("workflows", [])
    if not trigger_workflow:
        return json.dumps([], indent=2)
    r = await call_api(
        url=f"https://api.github.com/repos/{owner}/{repo}/contents/.github/workflows", params={}, raw=True
    )
    if isinstance(r, str):
        return json.dumps([r])
    if not r.json():
    ...
    for path in paths_list:
        ...
        active = await call_api(
            url=f"https://api.github.com/repos/{owner}/{repo}/actions/workflows/{workflow_id}", params={}
        )
        if not isinstance(active, str) and active.json().get("state") == "active":
            r = await call_api(url=f"https://api.github.com/repos/{owner}/{repo}/contents/{path}", params={}, raw=True)
            if isinstance(r, str):
                return json.dumps([r])
            data = yaml.safe_load(r.text)
            name = data.get("name", "")
            if name in trigger_workflow or "*" in trigger_workflow:
                ...
                if not high_privileged or high_privileged_triggers.intersection(set(triggers)):
                    results.append({"path": path, "name": name, "triggers": triggers})
...
async def get_high_privileged_workflow_triggers(
    ...
    for trigger in triggers:
        if isinstance(trigger, str):
            ...
            this_results = {
                ...
                or (
                    k == "workflow_run"
                    and (not v or isinstance(v, str) or (isinstance(v, dict) and "branches" not in v))
                )
            }
            if this_results:
    ...
    return json.dumps(
        ["Workflow is high privileged" if results else "Workflow is not high privileged", results], indent=2
    )
...
async def get_workflow_user(
    ...
    *,
    ...
    paths = await call_api(url=f"https://api.github.com/repos/{owner}/{repo}/contents/.github/workflows", params={})
    if isinstance(paths, str) or not paths.json():
        return json.dumps([], indent=2)

    paths_list = [
        item["path"] for item in paths.json() if item["path"].endswith(".yml") or item["path"].endswith(".yaml")
    ]
    results = []
    for path in paths_list:
        r = await call_api(url=f"https://api.github.com/repos/{owner}/{repo}/contents/{path}", params={}, raw=True)
        ...
        for i, line in enumerate(lines):
            for use, line_list in actual_name.items():
                if use in line:
                    line_list.append(i + 1)
        for line_numbers in actual_name.values():
            if not line_numbers:
                continue
            results.append(
                {"user": path, "lines": line_numbers, "action_name": workflow_file_path, "repo": f"{owner}/{repo}"}
            )
